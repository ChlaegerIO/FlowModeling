...
params['number_epoch_ae_start'] = 4001                       # train interval [Ae_start + Sindy_start, Ae_end + Sindy_end]
params['number_epoch_ae_end'] = 5601
    ...
    # train with autoencoder and sindy
        ...
        for batch_id, img_tensor in enumerate(train_data):
            ...
            if batch_id == train_idxOfNewVideo[pos]:
                # store z states of old video, only shift video by one frame --> only next frame prediction, not next batch!!!
                z_coord = np.concatenate((z_coord, z_tmp[0:len(z_tmp)-1]))
                dz_coord = np.concatenate((dz_coord, z_tmp[1:len(z_tmp)]))
                z_tmp = np.empty((0, params['z_dim']))
...
# note that both versions should work, with z10 I had some problems of overflowing
                ######## 1st approach: make several steps for sindy prediction of next batch
                for i in range(params['batch_size']):
                    ...
                    network['z'] = network['dz_sindy']

######### 2nd approach: only take last z as starting point for prediction
